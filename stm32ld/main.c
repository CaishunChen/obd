...
#define CHIP_ID_ALT           0x0422

int STM32_FLASH_START_ADDRESS = 0x08000000;
// ****************************************************************************
...
static u32 writeh_read_data1( u8 *dst, u32 len )
{
  size_t readbytes = 0;

  if( !feof( fp1 ) )
    readbytes = fread( dst, 1, len, fp1 );
  return ( u32 )readbytes;
}
...
static void writeh_progress1( u32 wrote )
{
  unsigned pwrite = ( wrote * 100 ) / fp1size;
  static int expected_next = 10;
...
int main( int argc, const char **argv )
{
  ...
  fprintf(stderr,"Program Address: %x\n",STM32_FLASH_START_ADDRESS);
 
  if( argc >= 7 && strlen(argv[ 6 ])==1 && strncmp(argv[ 6 ], "1", 1)==0 )
  {
    send_go_command=1;
  }
  ...
  if( argc == 6)
   {
  	STM32_FLASH_START_ADDRESS = strtoul(argv[4], 0, 16); 
  fprintf(stderr,"Program Address: %x\n",STM32_FLASH_START_ADDRESS);
    if( stm32_write_flash( writeh_read_data1, writeh_progress1 ) != STM32_OK )
    {
      fprintf( stderr, "Unable to program FLASH memory.\n" );
      exit( 1 );
    }
